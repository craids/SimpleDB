...
public class LockManager {
	...
	private final ConcurrentMap<PageId, Object> locks = new ConcurrentHashMap<>();
	private final Map<PageId, List<TransactionId>> sharedLocks = new HashMap<>();
	private final Map<PageId, TransactionId> exclusiveLocks = new HashMap<>();
	private final ConcurrentMap<TransactionId, Collection<PageId>> pageIdsLockedByTransaction = new ConcurrentHashMap<>();
	private final ConcurrentMap<TransactionId, Collection<TransactionId>> dependencyGraph = new ConcurrentHashMap<>();
	...
	public boolean acquireLock(TransactionId transactionId, PageId pageId, Permissions permissions)
		...
		TransactionId notNullTransactionId = (transactionId == null) ? TransactionId.NULL_TID : transactionId;
		if (permissions == Permissions.READ_ONLY) {
			...
			throw new IllegalArgumentException("Did not find READ_ONLY or READ_WRITE permissions.");
		}
		addPageToTransactionLocks(notNullTransactionId, pageId);
		return true;
	}

	public boolean acquireReadOnlyLock(TransactionId transactionId, PageId pageId) throws TransactionAbortedException {
	...
	public boolean acquireReadWriteLock(TransactionId transactionId, PageId pageId) throws TransactionAbortedException {
		Object lock = getLock(pageId);
		while (true) {
			synchronized (lock) {
	...
	private void abortOnDeadlock() throws TransactionAbortedException {
		...
		for (TransactionId transactionId : dependencyGraph.keySet()) {
			if (!visitedTransactionIds.contains(transactionId))
				attemptDeadlock(transactionId, visitedTransactionIds, new Stack<TransactionId>());
		}
	}

	private void attemptDeadlock(TransactionId transactionId, Set<TransactionId> visitedTransactionIds,
			Stack<TransactionId> parents) throws TransactionAbortedException {
		//depth-first search for directed cycles
		...
		for (TransactionId dependee : dependencyGraph.get(transactionId)) {
			...
			if (!visitedTransactionIds.contains(dependee)) {
				...
				attemptDeadlock(dependee, visitedTransactionIds, parents);
				parents.pop();
			}
		}
	}
	...
	private Collection<TransactionId> getLockHolders(PageId pageId) {
		ArrayList<TransactionId> lockHolders = new ArrayList<>();
	...
	private boolean isLockedByOthers(TransactionId transactionId, Collection<TransactionId> lockHolders) {
		if (lockHolders == null || lockHolders.isEmpty())
			return false;
		return !(lockHolders.size() == 1 && transactionId.equals(lockHolders.iterator().next()));
	}
	...
	public boolean holdsLock(TransactionId transactionId, PageId pageId) {
		return (!pageIdsLockedByTransaction.containsKey(transactionId)) ? false
				: pageIdsLockedByTransaction.get(transactionId).contains(pageId);
	}
}