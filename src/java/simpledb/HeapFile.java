...
public class HeapFile implements DbFile {
	...
	public HeapFile(File f, TupleDesc td) {
		...
		this.td = td;
		this.numPages = new AtomicInteger((int) file.length() / BufferPool.getPageSize());
	}
	...
	public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
		...
		for (int pageNum = 0; pageNum < numPages.get(); pageNum++) {
			insertedPage = tryInsertTuple(tid, t, pageNum);
			if (insertedPage != null) 
				break;
		}
		if (insertedPage == null) {
			int pageNum = numPages.getAndIncrement(); //not inserted
			HeapPage hpage = new HeapPage(new HeapPageId(getId(), pageNum), HeapPage.createEmptyPageData());
			writePage(hpage);
			insertedPage = tryInsertTuple(tid, t, pageNum);
		}
	...
	private HeapPage tryInsertTuple(TransactionId tid, Tuple t, int pageNumber)
	...
	class HeapFileIterator extends AbstractDbFileIterator {
		...
		protected Tuple readNext() throws TransactionAbortedException, DbException {
			...
			while (m_tupleIt == null && m_currentPageNumber < m_heapFile.numPages() - 1) {
				...
				HeapPage currentPage = (HeapPage) Database.getBufferPool().getPage(m_tid, currentPageId,
