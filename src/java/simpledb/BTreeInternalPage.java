...
public class BTreeInternalPage extends BTreePage {
	...
	/**
	 * Create a BTreeInternalPage from a set of bytes of data read from disk. The
	 * format of a BTreeInternalPage is a set of header bytes indicating the slots
	 * of the page that are in use, some number of entry slots, and extra bytes for
	 * the parent pointer, one extra child pointer (a node with m entries has m+1
	 * pointers to children), and the category of all child pages (either leaf or
	 * internal). Specifically, the number of entries is equal to:
	 * <p>
	 ...
	 * recordId is used to find the specified entry, so it must not be null. After
	 * deletion, the entry's recordId should be set to null to reflect that it is no
	 ...
	 * @param deleteRightChild - if true, delete the right child. Otherwise delete
	 *                         the left child
	 */
	 ...
	 * @throws DbException if this entry is not on this page, entry slot is already
	 *                     empty, or updating this key would put the entry out of
	 *                     order on the page
	 */
	 ...
	 * to reflect that it is now stored on this page.
	 * 
	 ...
	 *         this iterator throws an UnsupportedOperationException) (note that
	 *         this iterator shouldn't return entries in empty slots!)
	 */
